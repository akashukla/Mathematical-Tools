...
function Matrix:tonumber()
    return self:applyfunc(math.tonumber)
end
...
function Matrix:min(dim_or_mtx2)
    if dim_or_mtx2==nil or type(dim_or_mtx2)=='number' then
...
function Matrix:idiv(m)
    return self:binop(m,function(a,b) return math.floor(a/b) end)
end
...
function Matrix:power(k)
    ...
    assert(type(k)=='number','only number matrix power are allowed')
...
setmetatable(Matrix,{__call=function(self,rows,cols,data,t)
    ...
    assert(type(rows)=='number','rows must be an number')
    assert(type(cols)=='number','cols must be an number')
    ...
    if type(data)=='table' then
        ...
        elseif rows==-1 then
            rows=math.floor(#data/cols)
        elseif cols==-1 then
            cols=math.floor(#data/rows)
        else
...
if arg and #arg==1 and arg[1]=='test' then
    ...
    assert(math.floor(Vector{2.1,7,8.2}/2)==Vector{1,3,4})
